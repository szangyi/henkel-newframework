...
*/
		...
		const nextClickCover = document.createElement("div")
		...
		function stopAndGoCheck(){
			...
			if(stopId && stopPages.length) {
		        ...
		        next.style.cssText =  stop ? `
		                ...
		                cursor: not-allowed;
		        ` : '';
				next.querySelector('*:not(span)').style.zIndex = stop ? -1 : ''
			...
			// style nextClickCover
			nextClickCover.style.cssText = `
				...
				pointer-events: none;
			`
			next.insertAdjacentElement("afterend", nextClickCover)
			nextClickCover.addEventListener("click", ()=>{
				dispatchEvent(new CustomEvent("stopAndGoMessage"))
			})

			// next click cover
			nextClickCover.style.pointerEvents = stop ? "" : "none"
		}
